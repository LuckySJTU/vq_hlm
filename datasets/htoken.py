...
class MyDataset(GeneratorBasedBuilder):
    ...
    def _generate_examples(self, filepath):
        ...
        for i in range(0, len(raw_token)-1, 64):
            input_ids = raw_token[i:i+64]
            label = raw_token[i+1:i+65]
            if len(input_ids) > len(label):
                input_ids = input_ids[:len(label)]
            if len(input_ids) < 64:
                input_ids = torch.cat((input_ids, torch.zeros((64-input_ids.shape[0], input_ids.shape[1]), dtype=input_ids.dtype)))
                label = torch.cat((label, torch.zeros((64-label.shape[0], label.shape[1]), dtype=label.dtype)*(-100)))
                # input_ids = torch.cat((input_ids, raw_token[:64-input_ids.shape[0]]))
                # label = torch.cat((label, raw_token[1:65-label.shape[0]]))
