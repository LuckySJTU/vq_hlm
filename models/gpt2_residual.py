# In this model, we use residual lm head for multi-token prediction
...
class HLMGPT2(GPT2PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.residual_head = torch.nn.Sequential(*[
            torch.nn.Sequential(
                ...
                torch.nn.LeakyReLU(), # TODO, which activation or just no activation?
            )
            for _ in range(self.num_quantizer)])
        self.vq_lm_head = torch.nn.Linear(self.embedding_dim, self.codebook_size)
    ...
    def parallelize(self, device_map=None):
        ...
        self.residual_head.to(self.transformer.last_device)
        self.vq_lm_head.to(self.transformer.last_device)
    ...
    def deparallelize(self):
        ...
        self.vq_lm_head.to('cpu')
        self.model_parallel = False
        torch.cuda.empty_cache()

    def get_output_embeddings(self):
        return self.vq_lm_head

    def set_output_embeddings(self, new_embeddings):
        self.vq_lm_head = new_embeddings
    ...
    def forward(
        ...
        if self.model_parallel:
            ...
            hidden_states = hidden_states.to(self.vq_lm_head.weight.device)

        lm_logits = []
        loss = None
        labels = labels.to(self.vq_lm_head.weight.device)
        for i in range(self.num_quantizer):
            residual = self.residual_head[i](hidden_states)
            hidden_states = hidden_states - residual
            lm_logits.append(self.vq_lm_head(residual)) # Bx1024x8192
            if loss is None:
