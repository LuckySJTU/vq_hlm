...
class HLMGPT2Config(GPT2Config):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # self.n_positions = self.n_positions // chunk_size * self.num_quantizer # chunksize = 16
...
class HLMGPT2(GPT2PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.wte = torch.nn.Embedding(self.codebook_size, self.embedding_dim)
        # model.wte_proj = torch.nn.Linear(self.codebook_dim, self.embedding_dim)
        self.vqhead = torch.nn.Linear(self.embedding_dim, self.codebook_size)
    ...
    def forward(
        ...
        inputs_embeds = None
        input_ids = input_ids.view(input_ids.shape[0], -1) # b x (TxN)
        input_ids = input_ids.view(input_ids.shape[0]*self.num_quantizer, -1) # (bxN) x T
        if attention_mask is not None:
            # attention_mask: b x T, unmask: 1, mask: 0
            attention_mask = attention_mask.repeat_interleave(self.num_quantizer, dim=1) # b x (TxN)
            attention_mask = attention_mask.view(attention_mask.shape[0]*self.num_quantizer, -1) # (bxN) x T
        ...
        if self.model_parallel:
            ...
            hidden_states = hidden_states.to(self.vqhead.weight.device)

        labels = labels.to(self.vqhead.weight.device)
        # labels = labels.view(labels.shape[0], -1)
        # labels = labels.view(labels.shape[0]*self.num_quantizer, -1)

        lm_logits = self.vqhead(hidden_states)
        loss = F.cross_entropy(lm_logits.view(-1, lm_logits.shape[-1]), labels.reshape(-1).long(), ignore_index=-100)
