...
from models.gpt2_flatten import HLMGPT2, HLMGPT2Config
...
class HLTGPT2Config(GPT2Config):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.hlm_dir = vq_config.get('hlm_dir', None)
...
class HLTGPT2(GPT2PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        if config.vq_pretrained_checkpoint is not None and config.vq_pretrained_checkpoint != "":
            ...
            self.vq = nn.Identity()
            self.use_vq = False
        if config.hlm_dir is not None:
            print(f"Loading pretrained HLM from {config.hlm_dir}")
            self.hlm = HLMGPT2.from_pretrained(config.hlm_dir)
            for param in self.hlm.parameters():
                param.requires_grad = False
        else:
            self.hlm = nn.Identity()
            self.use_vq = False
    ...
    def hlm_transformer(
        ...
        for i in range(len(self.transformer.h)):
            ...
            if i==6 and self.use_vq:
                ...
                _, vq_indices, _ = self.vq(hidden_states[:, self.chunk_size-1::self.chunk_size, :]) # last token
                vq_attention_mask = attention_mask[:, :, self.chunk_size-1::self.chunk_size, self.chunk_size-1::self.chunk_size][:, 0, -1].clone()
                vq_attention_mask[vq_attention_mask==torch.finfo(self.transformer.dtype).min] = 1
                vq_attention_mask = 1 - vq_attention_mask
                pred_vq_logits = self.hlm(input_ids=vq_indices, attention_mask=vq_attention_mask, labels=vq_indices).logits
                pred_vq_indices = torch.argmax(pred_vq_logits, dim=-1)

                vq_indices = vq_indices.view(vq_indices.shape[0], -1)
                vq_indices = vq_indices.view(vq_indices.shape[0]*self.num_quantizer, -1)
                vq_target_labels = vq_indices[:, 1:]
                vq_pred_acc = torch.sum(vq_target_labels == pred_vq_indices).item() / (vq_target_labels.shape[0]*vq_target_labels.shape[1])
                pred_vq_indices = torch.cat((vq_indices[:,0].unsqueeze(dim=1), pred_vq_indices),dim=1) # (bxN) x T
                pred_vq_indices=pred_vq_indices.reshape(hidden_states.shape[0], -1, self.num_quantizer) # b x T x N

                out = self.vq.get_output_from_indices(pred_vq_indices) # b x T x D

                hidden_states = torch.cat((out, hidden_states), dim=1)
                # 对角线为1，重复chunk_size次，最后拼接原来的attention_mask
                hlm_attention_mask = attention_mask[:, :, self.chunk_size-1::self.chunk_size, self.chunk_size-1::self.chunk_size].clone()
